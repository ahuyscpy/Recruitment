...
namespace Application.Catalog
{
    public class UserService : IUserService
    {
        ...
        public async Task<ApiResult<bool>> GetFollowCompany(FollowCompanyViewModel listFollow)
        {
            var results = await _context.Follows.FirstOrDefaultAsync(x => x.UserId == listFollow.UserId && x.CompanyId == listFollow.CompanyId);
            if (results == null)
            {
        ...
        public async Task<ApiResult<bool>> UnFollowCompany(FollowCompanyViewModel listFollow)
        {
            var user = await _context.Follows.FirstOrDefaultAsync(x => x.UserId == listFollow.UserId && x.CompanyId == listFollow.CompanyId);
